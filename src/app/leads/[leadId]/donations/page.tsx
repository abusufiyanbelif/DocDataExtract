...
export default function DonationsPage() {
  ...
  const canCreate = userProfile?.role === 'Admin' || !!userProfile?.permissions?.leads?.donations?.create;
  const canUpdate = userProfile?.role === 'Admin' || !!userProfile?.permissions?.leads?.donations?.update;
  const canDelete = userProfile?.role === 'Admin' || !!userProfile?.permissions?.leads?.donations?.delete;
  ...
  const handleFormSubmit = async (data: DonationFormData) => {
    ...
    if (data.isTransactionIdRequired && data.transactionId && !editingDonation) {
        const isDuplicate = donations.some(d => d.transactionId === data.transactionId && d.leadId === leadId);
        if (isDuplicate) {
            toast({
