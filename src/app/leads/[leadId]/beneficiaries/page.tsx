...
export default function BeneficiariesPage() {
  ...
  const canCreate = userProfile?.role === 'Admin' || !!userProfile?.permissions?.leads?.beneficiaries?.create;
  const canUpdate = userProfile?.role === 'Admin' || !!userProfile?.permissions?.leads?.beneficiaries?.update;
  const canDelete = userProfile?.role === 'Admin' || !!userProfile?.permissions?.leads?.beneficiaries?.delete;
  ...
  const handleDeleteConfirm = async () => {
    if (!beneficiaryToDelete || !firestore || !storage || !leadId || !canDelete) return;
  ...
  const handleImportBeneficiaries = async () => {
    if (!selectedFile || !firestore || !leadId || !canCreate || !userProfile) return;
    setIsImporting(true);
        ...
        try {
            ...
            await batch.commit()
                .catch((serverError: any) => {
                    const permissionError = new FirestorePermissionError({
                        path: `leads/${leadId}/beneficiaries`,
                        operation: 'write',
  ...
  const handleSyncKitAmounts = async () => {
    if (!firestore || !lead || !beneficiaries || !canUpdate) {
    ...
    const { rationLists } = lead;
    if (!rationLists || Object.keys(rationLists).length === 0) {
        toast({ title: 'Sync Canceled', description: 'No item lists found for this lead to calculate amounts.', variant: 'destructive' });
        setIsSyncing(false);
        return;
    }
    ...
    for (const beneficiary of beneficiaries) {
        ...
        if (beneficiary.kitAmount !== expectedAmount) {
            const docRef = doc(firestore, `leads/${leadId}/beneficiaries`, beneficiary.id);
