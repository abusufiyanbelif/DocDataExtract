...
import { User, Loader2, Download, Wand2 } from 'lucide-react';
...
export function IdentityExtractor() {
  ...
  const [result, setResult] = useState<ExtractKeyInfoFromIdentityDocumentOutput | null>(null);
      ...
      <Card>
        ...
        <CardContent className="flex flex-col items-center gap-6">
          ...
          <Button onClick={handleScan} disabled={!photoDataUri || isLoading} className="w-full">
            {isLoading ? <Loader2 className="animate-spin" /> : 'Extract Info'}
          </Button>
        </CardContent>
      </Card>
      <Card>
        ...
        <CardContent className="space-y-4">
          ...
          {result && (
            <div className="space-y-4">
              <ResultDisplay label="Full Name" value={result.name} />
              <ResultDisplay label="Address" value={result.address} />
              <ResultDisplay label="ID Number" value={result.idNumber} />
            </div>
          )}
          {!isLoading && !result && (
