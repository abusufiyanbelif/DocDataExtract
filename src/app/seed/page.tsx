...
export default function SeedPage() {
    ...
    const seedDatabase = async () => {
        ...
        if (allUsers.some(u => u.userKey === adminData.userKey)) {
            ...
            batch.set(doc(usersCollectionRef), { ...adminData, createdAt: serverTimestamp() });
        ...
        if (allUsers.some(u => u.userKey === sampleUserData.userKey)) {
            ...
            batch.set(doc(usersCollectionRef), { ...sampleUserData, createdAt: serverTimestamp() });
            log(" -> Sample user data prepared.");
        }
        
        // --- Create Storage Folders ---
        log(" -> Creating folders in Firebase Storage...");
        await uploadString(storageRef(storage, `campaigns/.placeholder`), '', 'raw');
        await uploadString(storageRef(storage, `users/.placeholder`), '', 'raw');
        log("   -> SUCCESS: Root storage paths ensured.");
        ...
        if (campaignDocSnapshot.empty) {
            ...
            await uploadString(storageRef(storage, `campaigns/${campaignId}/beneficiaries/.placeholder`), '', 'raw');
            await uploadString(storageRef(storage, `campaigns/${campaignId}/donations/.placeholder`), '', 'raw');
                        ...
                        <div className="flex flex-col sm:flex-row gap-4">
                         ...
                         {errorOccurred && (
                            <Alert variant="destructive">
                                ...
                                <AlertDescription>
                                    An error occurred during the seeding process. Please review the logs below. If the error is related to permissions, use the System Diagnostics page to get specific guidance.
                                </AlertDescription>
                            </Alert>
                        )}
