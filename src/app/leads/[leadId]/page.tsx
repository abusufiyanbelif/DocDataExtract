...
export default function LeadDetailsPage() {
  ...
  const masterPriceList = useMemo(() => {
    const lists = editableLead?.rationLists;
    if (!lists) return {};
  ...
  const canReadSummary = userProfile?.role === 'Admin' || !!get(userProfile, 'permissions.leads.summary.read', false);
  const canReadRation = userProfile?.role === 'Admin' || !!get(userProfile, 'permissions.leads.ration.read', false);
  const canReadBeneficiaries = userProfile?.role === 'Admin' || !!get(userProfile, 'permissions.leads.beneficiaries.read', false);
  const canReadDonations = userProfile?.role === 'Admin' || !!get(userProfile, 'permissions.leads.donations.read', false);
  const canUpdate = userProfile?.role === 'Admin' || get(userProfile, 'permissions.leads.update', false) || get(userProfile, 'permissions.leads.ration.update', false);
  ...
  const handleDownload = async (format: 'csv' | 'excel' | 'pdf') => {
    if (!lead) {
      toast({ title: 'No Data', description: 'There is no lead data to export.', variant: 'destructive' });
      return;
    }
    
    const { rationLists, priceDate, shopName, shopContact, shopAddress } = lead;
    ...
    if (format === 'csv' || format === 'excel') {
        ...
        if (format === 'excel') {
            XLSX.writeFile(wb, `item-details-${priceDate}.xlsx`);
        } else {
            ...
            if (firstSheetName) {
                ...
                link.download = `item-details-${priceDate}.csv`;
        ...
        doc.setFontSize(14);
        doc.text(`${lead.name} - Item Details`, 14, startY);
        startY += 8;
        ...
        doc.save(`item-details-${priceDate}.pdf`);
    }
  };
  ...
  const handleAddNewCategory = () => {
    ...
    if (!newMemberCount || isNaN(Number(newMemberCount)) || Number(newMemberCount) <= 0) {
        toast({ title: 'Invalid Input', description: 'Please enter a valid positive number for members.', variant: 'destructive' });
        return;
    }
    const newCountStr = String(Math.floor(Number(newMemberCount)));
    if (editableLead.rationLists[newCountStr]) {
        toast({ title: 'Category Exists', description: `A category for ${newCountStr} members already exists.`, variant: 'destructive' });
        return;
    }
    
    const newRationLists = { ...editableLead.rationLists, [newCountStr]: [] };
    handleFieldChange('rationLists', newRationLists);
    setNewMemberCount('');
  ...
  const memberCategories = useMemo(() => {
    ...
    return Object.keys(editableLead.rationLists).sort((a, b) => {
        const aIsGeneral = a.toLowerCase().includes('general');
        const bIsGeneral = b.toLowerCase().includes('general');
  ...
  const sourceCategoriesForCopy = useMemo(() => {
    if (!editableLead || !copyTargetCategory) return [];
    return Object.keys(editableLead.rationLists).filter(
      (cat) => cat !== copyTargetCategory && editableLead.rationLists[cat].length > 0
    );
  }, [editableLead, copyTargetCategory]);
  
  const itemsAvailableToCopy = useMemo(() => {
    if (!editableLead || !copyTargetCategory || !copySourceCategory) return [];
    
    const currentItemNames = new Set(
        (editableLead.rationLists[copyTargetCategory] || [])
        .map(item => item.name.toLowerCase().trim())
        .filter(Boolean)
    );

    const sourceItems = editableLead.rationLists[copySourceCategory] || [];

    return sourceItems.filter(item => {
        const trimmedName = item.name.toLowerCase().trim();
        return trimmedName && !currentItemNames.has(trimmedName);
    });
  }, [editableLead, copyTargetCategory, copySourceCategory]);

  const handleConfirmCopy = () => {
    if (!editableLead || !copyTargetCategory || itemsToCopy.length === 0) return;
          ...
          <CardHeader>
             <div className="flex justify-between items-start flex-wrap gap-4">
                ...
                <div className="flex gap-2 flex-wrap justify-end">
                    ...
                    {canUpdate && editMode && editableLead.category === 'Ration' && (
                        <Dialog open={isAddCategoryOpen} onOpenChange={setIsAddCategoryOpen}>
                            ...
                            <DialogContent className="sm:max-w-[425px]">
                                <DialogHeader>
                                    ...
                                    <DialogDescription>
                                        Enter the number of family members for the new item list.
                                    </DialogDescription>
                                </DialogHeader>
             ...
             {editableLead.category === 'Ration' ? (
                memberCategories.length > 0 ? (
                    ...
                    <div className="text-center text-muted-foreground py-10">
                        No item categories defined for this lead yet.
